...
import { ArrowLeft, Shield, Search, RefreshCw } from 'lucide-react';
...
export default function MintRequests() {
  ...
  const { getRequest, getSignatures, addSignature, updateRequestStatus, getPendingRequests } =
    useMintRequests();
          ...
          <div className="flex items-center gap-4">
            <Link to="/simulator">
              <Button variant="ghost" size="sm" className="gap-2">
                <ArrowLeft className="h-4 w-4" /> Simulator
              </Button>
            </Link>
            <div className="flex items-center gap-2">
              <Shield className="h-5 w-5 text-primary" />
              <h1 className="text-lg font-bold">Mint Requests</h1>
            </div>
          </div>
